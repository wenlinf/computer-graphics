...
void main()
{
    // define different height levels
    int SNOW_LEVEL = 90;
    ...
    int SOIL_LEVEL = 25;
    int WATER_LEVEL = 10;
    int DEEP_WATER_LEVEL = 5;

    // Store our final texture color
    vec3 diffuseColor;
    if (FragPos.y > SNOW_LEVEL) {
        diffuseColor = vec3(1.000, 1.000, 1.000);
    } else if (FragPos.y > SUB_SNOW_LEVEL && FragPos.y <= SNOW_LEVEL) {
        diffuseColor = vec3(1.000, 0.980, 0.980);
    } else if (FragPos.y > LIGHT_GREEN_LEVEL && FragPos.y <= SUB_SNOW_LEVEL) {
    ...
    } else if (FragPos.y > WATER_LEVEL && FragPos.y <= SOIL_LEVEL) {
        diffuseColor = vec3(0.255, 0.412, 0.882);
    } else {
        diffuseColor = vec3(0.000, 0.000, 0.545);
